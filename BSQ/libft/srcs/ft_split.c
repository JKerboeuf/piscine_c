...
/*   Updated: 2019/07/23 19:28:34 by jukerboe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_strdupn(char *str, char *charset, int i)
{
	int		j;
	int		tmp;
	char	*res;
...
static char	**alloc(char *str, char *charset, int *i, int *j)
{
	int		size;
	char	**res;
	...
	{
		res[*j] = ft_strdupn(str, charset, *i);
		*j = *j + 1;
	}
	res[size] = 0;
	return (res);
...
char		**ft_split(char *str, char *charset)
{
	...
	while (str[i] && str[i + 1])
	{
		if (is_sep(str[i], charset) && !is_sep(str[i + 1], charset))
		{
			res[j] = ft_strdupn(str, charset, i + 1);
			j++;
		}
		i++;
	}
